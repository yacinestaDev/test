...
class _ReminderScreenState extends State<ReminderScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: appBarWidget(languages.lblDailyReminders,
            context: context,
            actions: [
              IconButton(
                onPressed: () async {
        ...
        body: notificationStore.mRemindList.isNotEmpty
            ? AnimatedListView(
                ...
                itemBuilder: (context, index) {
                  DateTime? duration = DateTime.parse(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Text(
                                "${weekdays[notificationStore.mRemindList[index].week.toInt() - 1]} ${formattedTime.validate()}",
                            ...
                            Container(
                                ...
                                .paddingAll(4)
                                .onTap(() {
                              // AwesomeNotifications().cancel(notificationStore
                              //     .mRemindList[index].id
                              //     .validate());
                              notificationStore.mRemindList[index].status = 1;
                              print(
                                  notificationStore.mRemindList[index].status);
                        ...
                        Row(
                          children: [
                            Text(
                                "  ${notificationStore.mRemindList[index].title.validate()}",
                                style: boldTextStyle(size: 16).copyWith(
                                    color: textPrimaryColorGlobal
                                        .withOpacity(0.4))),
                            Text(
                                " | ${notificationStore.mRemindList[index].subTitle.validate()}",
